...
char* usage =
		"Usage: easier-dec [OPTION ...] PUB_KEY PRIV_KEY LAMBDA_FILE FILE \n"
		"\n"
		"Decrypt FILE using private key PRIV_KEY and assuming public key\n"
		"PUB_KEY and lambda_k LAMBDA_FILE. If the name of FILE is X.cpabe.proxy, the decrypted file will\n"
...
parse_args( int argc, char** argv )
{
	...
	if( !pub_file || !prv_file || !lambda_file || !in_file )
		die(usage);

	if( !out_file )
	{
		if( strlen(in_file) > 12 && !strcmp(in_file + strlen(in_file) - 12, ".cpabe.proxy") )
...
main( int argc, char** argv )
{
	...
	unserialize_element(suck_file(lambda_file), &offset, lambda_k);

	char* in_aes_file = g_strdup_printf("%s.cpaes", g_strndup(in_file, strlen(in_file) - 12));
	read_cph_file(in_file, &cph_buf);
	read_aes_file(in_aes_file, &file_len, &aes_buf);
